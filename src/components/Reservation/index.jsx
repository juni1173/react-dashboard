...
const Index = () => {
  ...
  const handleMonthChange = (event) => {
    ...
    const lastDay = dayjs(`${year}-${monthIndex}-01`)
      .endOf("month")
      .format("YYYY-MM-DD");
  ...
  async function getOrders() {
    ...
    const apiUrl = `${process.env.React_APP_API_URL}/wp-json`;
              ...
              {searchType === "range" && (
                <>
                  ...
                  <Grid item xs={12} style={{ textAlign: "center" }}>
                    <Button
                      onClick={() =>
                        updateSearch(dateRange.from, dateRange.to, "range")
                      }
                    >
                      Search Bookings
                    </Button>
          ...
          >
            ...
            {searchType ? (
              <Typography>
                No orders found{" "}
                {searchType === "date"
                  ? `on ${selectedDate ? selectedDate : "no date selected"}`
                  : `from ${
                      dateRange.from ? dateRange.from : "not selected"
                    } to ${dateRange.to ? dateRange.to : "not selected"}`}
              </Typography>
            ) : (
